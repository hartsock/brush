...
//!
//! These tests target unix, where the absolute paths used below
//! (`/bin/rm`, `/bin/true`, etc.) and external-spawn semantics are stable.
#![cfg(unix)]
#![cfg(test)]
...
async fn allows_permitted_command() -> Result<()> {
    ...
    // `/bin/true` is a path-separator command too, so this also proves the
    // Allow path of the path-separator branch.
    let code = run(&mut shell, "/bin/true").await?;
    assert_eq!(code, 0, "permitted `/bin/true` should succeed");

    let exec_calls = interceptor.exec_calls();
    assert!(
        exec_calls.iter().any(|p| p == "/bin/true"),
        "before_exec should have observed `/bin/true`; saw: {exec_calls:?}"
    );
    Ok(())
}
